...
- **Changes**
    ...
    - **Attester role:**
        - Attesters propagate their individual attestations to the `attestation_{subnet_id}` gossipsub topic
    ...
    - **Slot intervals:**
        - Intervals
            ...
            | 1 |  | - Create and propagate attestation to the attestation subnet | - Collect attestations from attestation subnet |
            | 2 |  |  | - Compute and propagate an aggregation from collected attestations |
            | 3 |  | - Compute safe target with 2/3+ majority |  |
            | 4 |  | - Accept accumulated attestations (new → known) <br />- Update head based on new attestation weights |  |
